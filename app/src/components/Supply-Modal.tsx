...
export const SupplyModal: FC<SupplyModalProps> = ({ row, open, onOpenChange, onDeposit }) => {
  ...
  const usdValue = parseFloat(row.totalSupplyUsd.replace(/[$,M]/g, ''))
  const minCollateralUsd = minCollateral * usdValue
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="mt-4 flex flex-col gap-2 text-sm text-slate-200">
          ...
          <div>
            <span className="font-medium text-white">Interest (APY):</span> {row.supplyApy}
          </div>
        </div>
