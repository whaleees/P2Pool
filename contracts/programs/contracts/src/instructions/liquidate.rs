...
pub fn liquidate_handler(
    ...
    let collateral_feed = load_price_feed_from_account_info(&ctx.accounts.collateral_price_feed).unwrap();
    let borrow_feed = load_price_feed_from_account_info(&ctx.accounts.borrow_price_feed).unwrap();

    let collateral_price = collateral_feed.get_current_price().ok_or(ErrorCode::InvalidPriceFeed)?.price() as u64;
    let borrow_price = borrow_feed.get_current_price().ok_or(ErrorCode::InvalidPriceFeed)?.price() as u64;

    // get the price in usd
    let collateral_value = pool_borrow.collateral_amount
        .checked_mul(collateral_price)
        .ok_or(ErrorCode::MathOverflow)?;

    let borrow_value = pool_borrow.borrow_amount
        .checked_mul(borrow_price)
        .ok_or(ErrorCode::MathOverflow)?;

    // calculate threshold >= 150%
    let liquidation_threshold = borrow_value
        .checked_mul(150)
        .ok_or(ErrorCode::MathOverflow)?
        .checked_div(100)
        .ok_or(ErrorCode::MathOverflow)?;
